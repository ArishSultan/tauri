...
impl Settings {
  pub fn new(current_dir: PathBuf, matches: &ArgMatches<'_>) -> crate::Result<Self> {
    ...
    let merged_bundle_settings = match tauri_config {
      Ok(config) => merge_settings(bundle_settings, config),
  ...
  pub fn build_tool(&self) -> Option<&String> {
    return self.bundle_settings.build_tool.as_ref()
  }
...
fn merge_settings(
  bundle_settings: BundleSettings,
  config: crate::bundle::tauri_config::Config,
) -> BundleSettings {
  let bundle_config = config.tauri.bundle;
  BundleSettings {
    ...
    short_description: options_value(bundle_config.short_description, bundle_settings.short_description),
    long_description: options_value(bundle_config.long_description, bundle_settings.long_description),
    ...
    build_tool: options_value(config.build.tool, bundle_settings.build_tool),
    ..bundle_settings
  }
}
